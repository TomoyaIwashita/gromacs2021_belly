 ...
 */
#include "gmxpre.h"
#include <stddef.h> //modi by T.Iwashita
#include <cinttypes>
#include <cmath>
#include <cstdio>
#include <cstdlib>
...
#include "gromacs/fileio/trrio.h"      //modi by T.Iwashita
...
void gmx::LegacySimulator::do_md()
{
    ...
    //modi by T.Iwashita ~849
    const bool        breadcf = opt2bSet("-readcf", nfile, fnm);
    const auto* const cffilename = opt2fn("-readcf", nfile, fnm);
    const auto* const rsfilename = opt2fn("-writers", nfile, fnm);
    bool              bcalF = FALSE;
    bool              bnextcf = FALSE;
    bool              bLastinner = FALSE;
    int64_t           lifeinner = 0, step_inner = 0, step_frame=1;
    const int         qfacter=mdrunOptions.qfacter;
    int               sumn=0;
    
    j=0;
    if (breadcf)
    {
       const int top_global_natoms = top_global->natoms;
       for (i = 0, j = 0; (i < top_global_natoms); i++)
       {
          const int res_getGroupType = getGroupType(top_global->groups, SimulationAtomGroupType::Freeze, i);
          if (res_getGroupType == 0)
          {
              j++;
          }
       }
    }
    const int         natoms_freeze = j;
    fprintf(stderr, "count freeze atom : %d.\n", natoms_freeze);

    t_fileio* cffio = nullptr;
    t_fileio* rsfio = nullptr;
    
    rvec* nowcf=nullptr;
    rvec* nextcf=nullptr;
    rvec* tmpreadv=nullptr;
    rvec* tmpreadf=nullptr;
    rvec* average_F=nullptr;
    if (breadcf){
       cffio = gmx_trr_open(cffilename, "r");
       rsfio = gmx_trr_open(rsfilename, "w");
       snew_aligned(nowcf, natoms_freeze, sizeof(real));
       snew_aligned(nextcf, natoms_freeze, sizeof(real));
       snew_aligned(tmpreadv, natoms_freeze, sizeof(real));
       snew_aligned(tmpreadf, natoms_freeze, sizeof(real));
       snew_aligned(average_F, natoms_freeze, sizeof(real));
       step_inner = 0;
       step_frame = 1;
       lifeinner  = 0;

       //Read the first structure from the initial structure
       auto x = makeArrayRef(state_global->x);
       fprintf(stderr, "set_nowcf.\n");
       const int top_global_natoms = top_global->natoms;
       for (i = 0, j = 0; (i < top_global_natoms); i++)
       {
          const int res_getGroupType = getGroupType(top_global->groups, SimulationAtomGroupType::Freeze, i);
       ...
       matrix tmpbox;
       int tmpnatoms;
       //Read the next structure from the cf file
       //Skip some frame if need.
       if (mdrunOptions.ncfskip > 0)
       {
          fprintf(stderr, "skip %d cf_frame.\n", mdrunOptions.ncfskip);
          const int ncfskip = mdrunOptions.ncfskip;
          for (i = 0; (i < ncfskip); i++)
          {
             if ( !gmx_trr_read_frame(cffio, &tmpstep, &tmpt, &tmplambda, tmpbox, &tmpnatoms, nextcf, tmpreadv, tmpreadf) )
             {
                bLastStep = TRUE;
             }
          }
       ...
       fprintf(stderr, "The number of atoms in cf_frane is %d \n", tmpnatoms);

       //Calculate n_prd
       real tmpmaxr=0;
       real memor=0;
       real tmp;
       fprintf(stderr, "set_lifeinner.\n");
       for (i = 0; (i < natoms_freeze); i++)
       {
          memor=0;
          for (m = 0; m < DIM; m++)
          {
             tmp=(nowcf[i][m]-nextcf[i][m]);
             memor+=tmp*tmp;
          }
          if (memor > tmpmaxr)
          {
       ...
       lifeinner=std::round( std::sqrt(tmpmaxr)*qfacter );
       fprintf(stderr, "first lifeinner=%d.\n", lifeinner);
       ...
       if (lifeinner==0)
       {
          bcalF=TRUE;
          bnextcf=FALSE;
          lifeinner=mdrunOptions.numStepslinner;
          fprintf(stderr, "init average_F. lifeinner=%d\n", lifeinner);
          for (i = 0; (i < natoms_freeze); i++)
          {
             for (m = 0; m < DIM; m++)
             {
       ...
       {
          gmx_fatal(FARGS,
                      "Not support domain decomposition.");
       }
       else
       {
          fprintf(stderr, "start_md. qfacter_inv=%f nm. \n", 1.0/qfacter);
       }
    }
	...
	//modi by T.Iwashita ~899
	if (breadcf)
	{
	   bLastinner = (step_inner == lifeinner);
           bLastStep = (step_frame == ir->nsteps) && bLastinner && bcalF;
	}
        else
	{
	...
	if (breadcf)
	{
	  ...
	  {
	      gmx_fatal(FARGS,
                      "don't support domain decomposition ");
	  }

	  //Caluculate the next configuration
	  
	  //if (bnextcf)
	  //{
	    //fprintf(stderr, "check");
	    //auto refx = makeArrayRef(state->x).subArray(0,mdatoms->homenr);
	    //fprintf(stderr, "%d, %f, %f, %f \n",step_inner,refx[0][0],refx[0][1],refx[0][2]);
            //for (i = 0, j = 0; (i < top_global->natoms); i++)
	    //{
              //const int res_getGroupType = getGroupType(top_global->groups, SimulationAtomGroupType::Freeze, i);
              //if (res_getGroupType == 0)
              //{
	      //  fprintf(stderr, "ms %d, %d, %f, %f, %f \n",i,j,refx[i][0],refx[i][1],refx[i][2]);
              //}
	    //} 
	  //}
	  ...
	  if (bnextcf)
	  {
            //fprintf(stderr, "start_nextcf\n"); 
	    const real par_nowconf=static_cast<real>(lifeinner-step_inner)/static_cast<real>(lifeinner);
	    const real par_nextconf=static_cast<real>(step_inner)/static_cast<real>(lifeinner);
            const int top_global_natoms = top_global->natoms;
	 
            rvec* x_coord = state->x.rvec_array();
	    for (i = 0, j = 0; (i < top_global_natoms); i++)
	    {
              const int res_getGroupType = getGroupType(top_global->groups, SimulationAtomGroupType::Freeze, i);
              //fprintf(stderr, "%d, %d, %d \n",i,j,res_getGroupType);
              if (res_getGroupType == 0)
              {
	  	for (m = 0; m < DIM; m++)
                {
		  x_coord[i][m] = par_nowconf*nowcf[j][m] + par_nextconf*nextcf[j][m];
                }
		j++;
              }
	    } 
	  ...
	  //Average the forces on freezed solute.
	  if (bcalF)
	  {
	    sumn++;
	    const rvec* tmpf = as_rvec_array(f.view().force().data());
            const int top_global_natoms = top_global->natoms;
	    ...
	    for (i = 0, j = 0; (i < top_global_natoms); i++)
            {
              ...
              if (res_getGroupType == 0)
              {
                 for (m = 0; m < DIM; m++)
                 {
		   average_F[j][m] += tmpf[i][m];
                 }
                 j++;
              }
	    }
	  
	     //If last inner step, write the averaged forces in fs file.
	    if (bLastinner)
            {
	      fprintf(stderr, "step_frame=%d, write average. sumn=%d",step_frame,sumn);
	      rvec* tmpx = nullptr;
	      auto refx = makeArrayRef(state->x);
	      snew(tmpx, natoms_freeze);
	      for (i = 0, j = 0; (i < top_global->natoms); i++)
              {
                const int res_getGroupType = getGroupType(top_global->groups, SimulationAtomGroupType::Freeze, i);
		if (res_getGroupType == 0)
                {
		  copy_rvec(refx[i], tmpx[j++]);
	        }
	      }
		...
		for (m = 0; m < DIM; m++)
	      ...
	      rvec* tmpv = nullptr;
	      gmx_trr_write_frame(rsfio, step_frame, 0, 0,
				  state->box, natoms_freeze, tmpx, tmpv, average_F);
	      sfree(tmpx);
	      fprintf(stderr, "end(0).\n");
	    }
	  }
	}
	...
	//modi by T.Iwashita
        step_inner++;
        if (!bLastStep && bLastinner && breadcf)
        {
	  fprintf(stderr, "switch, step_frame=%d, step_inner=%d",step_frame,step_inner);
	  step_inner=0;
	  if (bcalF)
          {
	    step_frame++;
    ...
    if (breadcf){
       sfree_aligned(nowcf);
       sfree_aligned(nextcf);
       sfree_aligned(tmpreadv);
       sfree_aligned(tmpreadf);
       sfree_aligned(average_F);
       gmx_trr_close(cffio);
       gmx_trr_close(rsfio);
    }
